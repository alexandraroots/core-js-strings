...
function findLongestWord(sentence) {
  ...
  for (let i = 0; i < words.length; i += 1) {
    ...
    if (currentWord.length > maxLenght) {
      maxLenght = currentWord.length;
      maxWord = currentWord;
    }
  }
  return maxWord;
...
function reverseWords(str) {
  ...
  for (let i = 0; i < strArray.length; i += 1) {
    ...
    for (let j = 0; j < word.length; j += 1) {
      reverseWord += word.charAt(word.length - 1 - j);
    }
...
function extractNameFromTemplate(value) {
  const match = value.match(/^Hello,\s(.+)!$/);
  return match[1];
}
...
function extractEmails(str) {
  const emails = [];
  const emailsStr = str.split(';');
  for (let i = 0; i < emailsStr.length; i += 1) {
    emails.push(emailsStr[i]);
  }
  return emails;
}
